...
AUTO_PUSH="${AUTO_PUSH:-0}"                    # 1 = dá git push após cada commit de cada agente
...
run_agent() {
  ...
  case "$speaker" in
    gemini) "$GEMINI_BIN" -y -p "$prompt" 2>&1 ;;
    claude) "$CLAUDE_BIN" -p "$prompt" --permission-mode "$CLAUDE_PERMISSION_MODE" 2>&1 ;;
  esac
}
